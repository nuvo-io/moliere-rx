...
object DdsObservable {
  ...
  def fromReaderData[T](dr: DataReader[T]): Observable[T] = Observable.create((o: Observer[T]) => {
    val lid = dr.listen {
      case DataAvailable(_) =>
        val state =
          dr.getParent.createDataState()
          .`with`(InstanceState.ALIVE)
          .`with`(SampleState.NOT_READ)
          .`with`(SampleState.READ)
          .withAnyViewState()
        dr.select().dataState(state).take().foreach(s => {
          o.onNext(s.getData)
        })
  ...
  def fromReaderEvents[T](dr: DataReader[T]): Observable[ReaderEvent[T]] =
